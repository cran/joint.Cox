...
function(t.event,event,t.death,death,Z1,Z2,group,
           alpha=1,p0=rep(0,13),kappa_grid=c(seq(10,1e+17,length=30)),
    ...
    l.func=function(phi){
      ...
      for(i in G_id){
        ...
        func1=function(u){
          ...
          uR1=rowSums( u%*%t( R1*exp(beta1*Z1[Gi]) ) )
          uR2=rowSums( (u^alpha)%*%t( R2*exp(beta2*Z2[Gi]) ) )
    ...
    if(temp){V=solve( H_PL+diag(rep(0.0001,13)) )}else{V=solve(H_PL)}
    
    conv=res$code
    iteration=res$iterations
    ML=-res$minimum
    
    beta1_est=res$est[12]
    beta2_est=res$est[13]
    g_est=exp(res$est[c(1,2,3,4,5)])
    h_est=exp(res$est[c(6,7,8,9,10)])
    eta_est=exp(res$est[11])
  
    beta1_se=sqrt(V[12,12])
    beta2_se=sqrt(V[13,13])
    ...
    list(count=count,
         beta1=beta1_res,beta2=beta2_res,eta=eta_res,K1=K1_est,K2=K2_est,
